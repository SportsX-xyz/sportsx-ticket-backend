...
import { USER } from '@/modules/admin/core/constants/permissions'
import {
  AdminController,
  Permissions,
  Public,
  Refresh,
  UserInfo,
} from '@/modules/admin/core/decorators'
...
export class UserController {
  ...
  async login(@Req() req: FastifyRequest) {
    const accessToken = this.jwtService.sign(
      ...
      {
        expiresIn: this.configService.get('ADMIN_JWT_ACCESS_EXPIRES') || '30m',
      }
    )
    ...
    const refreshToken = this.jwtService.sign(
      ...
      {
        expiresIn: this.configService.get('ADMIN_JWT_REFRESH_EXPIRES') || '7d',
      }
    )
  ...
  async refresh(@Query('refreshToken') refreshToken: string) {
    try {
      ...
      const signedAccessToken = this.jwtService.sign(
        ...
        {
          expiresIn:
            this.configService.get('ADMIN_JWT_ACCESS_EXPIRES') || '30m',
        }
      )
      ...
      const signedRefreshToken = this.jwtService.sign(
        ...
        {
          expiresIn:
            this.configService.get('ADMIN_JWT_REFRESH_EXPIRES') || '7d',
        }
      )
